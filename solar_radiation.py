...
df=pd.read_csv (r"C:\Users\Asus\OneDrive - IIT Kanpur\Desktop\SolarPrediction.csv")
print(df)
print(df.info())
print(df.columns )
#corr=df.corr()
df.drop(["UNIXTime","Data","TimeSunRise","TimeSunSet"],inplace=True,axis=1)
print(df.info())
df["Time_hour"]=df["Time"].str.split(":").str[0]
df["Time_hour"]=df["Time_hour"].astype(int)
df["Time_min"]=df["Time"].str.split(":").str[1]
